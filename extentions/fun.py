...
@plugin.command
@lightbulb.command('random', 'Every command related to random pictures.')
...
async def joke(ctx: lightbulb.Context) -> None:
    ...
    embed = hikari.Embed(title=request.get('setup'), description=f"||{request.get('delivery')}||", color=get_setting('embed_color'))
...
async def riddle(ctx: lightbulb.Context) -> None:
    ...
    embed = hikari.Embed(title=request.get('riddle'), description=f"||{request.get('answer')}||", color=get_setting('embed_color'))
    await ctx.respond(embed)
...
#     request = request.get('memes')
    
#     index = random.randint(0, len(request))
    
#     embed = hikari.Embed(title=request[index].get('name'), color=get_setting('embed_color'))
#     embed.set_image(request[index].get('url'))
...
async def rand_fact(ctx: lightbulb.Context) -> None:
    ...
    embed = hikari.Embed(title='Random Useless Fact', description=request.get('text'), color=get_setting('embed_color'))
    await ctx.respond(embed)
...
async def hangman(ctx: lightbulb.Context, word: str, theme: str) -> None:
    embed = hikari.Embed(title=f'Hangman game has started!', description='This menu is only for hiding your command.', color=get_setting('embed_color'))
    await ctx.respond(embed, flags=hikari.MessageFlag.EPHEMERAL)
    
    word = word.lower()
    guessers = {}

    embed = hikari.Embed(title=f'{ctx.user} has started a Hangman game!', description='`A perfect game to hang your mans!`', color=get_setting('embed_color'))
    embed.set_image(r'''game_preview\hangman\0.png''')
...
class HangmanLobbyView(miru.View):
    ...
    async def join(self, button: miru.Button, ctx: miru.Context) -> None:
        ...
        embed = hikari.Embed(title=f'{self.game["author"]} has started a Hangman game!', description='`A perfect game to hang your mans!`', color=get_setting('embed_color'))
    ...
    async def on_timeout(self) -> None:      
        embed = hikari.Embed(title=f'Game has timed out!', color=get_setting('embed_color'))
        embed.add_field(name='__Game info__', value=f'Host: <@{self.game["author"].id}>\nWord Count: `{len(self.game["word"].replace(" ", "")):,}`\nTheme: **{self.game["theme"].capitalize()}**', inline=True)
...
class HangmanGameView(miru.View):
    ...
    async def select_leter_1(self, select: miru.Select, ctx: miru.Context):
        letter = select.values[0]
        
        if letter in self.guesses: # if player guessed that letter already
            embed = hikari.Embed(description='This letter has already been guessed!', color=get_setting('embed_error_color'))
            await ctx.respond(embed, flags=hikari.MessageFlag.EPHEMERAL)
            return
        ...
        if letter not in self.charIndex:
            ...
            embed = hikari.Embed(title=f"Incorrect! It's {self.guesser['name']} Turn!", description=f'`{ctx.user.username}#{ctx.user.discriminator} has guessed letter {letter.capitalize()}.`\n\nHost: <@{self.author}>\nGuessers: {", ".join(self.game["guessers"])}\nWord Count: `{len(self.game["word"].replace(" ", "")):,}`\nTheme: **{self.game["theme"].capitalize()}**', color=get_setting('embed_color'))
            ...
            embed.set_image(fr'''game_preview\hangman\{self.mistakes}.png''')
            embed.set_thumbnail(self.guesser['url'])

            if self.mistakes != 7:
                await ctx.edit_response(embed, components=self.build())
            else:
                embed = hikari.Embed(title=f"Incorrect, the word was {self.game['word'].capitalize()}! {self.game['author']} wins!", description=f'`{ctx.user.username}#{ctx.user.discriminator} has guessed letter {letter.capitalize()}.`\n\nHost: <@{self.author}>\nGuessers: {", ".join(self.game["guessers"])}\nWord Count: `{len(self.game["word"].replace(" ", "")):,}`\nTheme: **{self.game["theme"].capitalize()}**', color=get_setting('embed_color'))
                embed.add_field(name='__Current letters__', value=' '.join(self.wordVisual), inline=True)
                embed.add_field(name='__Guesses__', value=', '.join(self.guesses).upper() if len(self.guesses) > 0 else 'None', inline=True)
                embed.set_image(fr'''game_preview\hangman\{self.mistakes}.png''')
            ...
            embed = hikari.Embed(title=f"Correct! It's {self.guesser['name']} Turn!", description=f'`{ctx.user.username}#{ctx.user.discriminator} has guessed letter {letter.capitalize()}.`\n\nHost: <@{self.author}>\nGuessers: {", ".join(self.game["guessers"])}\nWord Count: `{len(self.game["word"].replace(" ", "")):,}`\nTheme: **{self.game["theme"].capitalize()}**', color=get_setting('embed_color'))
            embed.add_field(name='__Current letters__', value=' '.join(self.wordVisual), inline=True)
            embed.add_field(name='__Guesses__', value=', '.join(self.guesses).upper() if len(self.guesses) > 0 else 'None', inline=True)
            embed.set_image(fr'''game_preview\hangman\{self.mistakes}.png''')
            embed.set_thumbnail(self.guesser['url'])
            ...
            if r'''\_''' not in self.wordVisual:
                embed = hikari.Embed(title=f"Correct! The guessing team wins!", description=f'`{ctx.user.username}#{ctx.user.discriminator} has guessed letter {letter.capitalize()}.`\n\nHost: <@{self.author}>\nGuessers: {", ".join(self.game["guessers"])}\nWord Count: `{len(self.game["word"].replace(" ", "")):,}`\nTheme: **{self.game["theme"].capitalize()}**', color=get_setting('embed_color'))
                embed.add_field(name='__Current letters__', value=' '.join(self.wordVisual), inline=True)
                embed.add_field(name='__Guesses__', value=', '.join(self.guesses).upper() if len(self.guesses) > 0 else 'None', inline=True)
                embed.set_image(fr'''game_preview\hangman\{self.mistakes}.png''')
                embed.set_thumbnail(ctx.user.avatar_url if ctx.user.avatar_url != None else ctx.user.default_avatar_url)
    ...
    async def select_leter_2(self, select: miru.Select, ctx: miru.Context):
        letter = select.values[0]
    ...
    async def select_number_1(self, select: miru.Select, ctx: miru.Context):
        ...
        if number not in self.charIndex:
            ...
            embed = hikari.Embed(title=f"Incorrect! It's {self.guesser['name']} Turn!", description=f'`{ctx.user.username}#{ctx.user.discriminator} has guessed number {number}.`\n\nHost: <@{self.author}>\nGuessers: {", ".join(self.game["guessers"])}\nWord Count: `{len(self.game["word"].replace(" ", "")):,}`\nTheme: **{self.game["theme"].capitalize()}**', color=get_setting('embed_color'))
            embed.add_field(name='__Current letters__', value=' '.join(self.wordVisual), inline=True)
            embed.add_field(name='__Guesses__', value=', '.join(self.guesses).upper() if len(self.guesses) > 0 else 'None', inline=True)
            embed.set_image(fr'''game_preview\hangman\{self.mistakes}.png''')
            embed.set_thumbnail(self.guesser['url'])
            ...
            if self.mistakes != 7:
                ...
                embed = hikari.Embed(title=f"Incorrect, the word was {self.game['word'].capitalize()}! {self.game['author']} wins!", description=f'`{ctx.user.username}#{ctx.user.discriminator} has guessed number {number}.`\n\nHost: <@{self.author}>\nGuessers: {", ".join(self.game["guessers"])}\nWord Count: `{len(self.game["word"].replace(" ", "")):,}`\nTheme: **{self.game["theme"].capitalize()}**', color=get_setting('embed_color'))
                embed.add_field(name='__Current letters__', value=' '.join(self.wordVisual), inline=True)
                embed.add_field(name='__Guesses__', value=', '.join(self.guesses).upper() if len(self.guesses) > 0 else 'None', inline=True)
                embed.set_image(fr'''game_preview\hangman\{self.mistakes}.png''')
                embed.set_thumbnail(ctx.user.avatar_url if ctx.user.avatar_url != None else ctx.user.default_avatar_url)
            ...
            embed = hikari.Embed(title=f"Correct! It's {self.guesser['name']} Turn!", description=f'`{ctx.user.username}#{ctx.user.discriminator} has guessed number {number}.`\n\nHost: <@{self.author}>\nGuessers: {", ".join(self.game["guessers"])}\nWord Count: `{len(self.game["word"].replace(" ", "")):,}`\nTheme: **{self.game["theme"].capitalize()}**', color=get_setting('embed_color'))
            embed.add_field(name='__Current letters__', value=' '.join(self.wordVisual), inline=True)
            embed.add_field(name='__Guesses__', value=', '.join(self.guesses).upper() if len(self.guesses) > 0 else 'None', inline=True)
            embed.set_image(fr'''game_preview\hangman\{self.mistakes}.png''')
            embed.set_thumbnail(self.guesser['url'])
            ...
            if r'''\_''' not in self.wordVisual:
                embed = hikari.Embed(title=f"Correct! The guessing team wins!", description=f'`{ctx.user.username}#{ctx.user.discriminator} has guessed number {number}.`\n\nHost: <@{self.author}>\nGuessers: {", ".join(self.game["guessers"])}\nWord Count: `{len(self.game["word"].replace(" ", "")):,}`\nTheme: **{self.game["theme"].capitalize()}**', color=get_setting('embed_color'))
                embed.add_field(name='__Current letters__', value=' '.join(self.wordVisual), inline=True)
                embed.add_field(name='__Guesses__', value=', '.join(self.guesses).upper() if len(self.guesses) > 0 else 'None', inline=True)
                embed.set_image(fr'''game_preview\hangman\{self.mistakes}.png''')
                embed.set_thumbnail(ctx.user.avatar_url if ctx.user.avatar_url != None else ctx.user.default_avatar_url)
