...
class InfoMenu(miru.View):
    ...
    async def view_check(self, ctx: miru.ViewContext) -> bool:
        if ctx.user.id != self.owner_id:
            embed = hikari.Embed(description='You are not the owner of this card!', color=get_setting('general', 'embed_error_color'))
...
class FavoriteButton(miru.Button):
    ...
    async def callback(self, ctx: miru.ViewContext) -> None:
        ...
        if not result:
            embed = hikari.Embed(description='This card does not exist!', color=get_setting('general', 'embed_error_color'))
...
class SellButton(miru.Button):
    ...
    async def callback(self, ctx: miru.ViewContext) -> None:
        ...
        if favorite:
            embed = hikari.Embed(description=f'You cannot sell favorite cards!', color=get_setting('general', 'embed_error_color'))
            await ctx.respond(embed, flags=hikari.MessageFlag.EPHEMERAL, delete_after=10)
            return

        view = PromptView()
        
        embed = hikari.Embed(title=f'Are you sure?', description=f'You will get 🪙 {price} for selling {name}.', color=get_setting('general', 'embed_error_color'))
        ...
        if view.answer:
            ...
            await ctx.edit_response(hikari.Embed(description=f'You sold {name} for 🪙 {price}!', color=get_setting('general', 'embed_success_color')), components=[], flags=hikari.MessageFlag.EPHEMERAL)
        else:
            await ctx.edit_response(hikari.Embed(description=f'Selling process has been cancelled.', color=get_setting('general', 'embed_error_color')), components=[], flags=hikari.MessageFlag.EPHEMERAL)
...
async def open(ctx: lightbulb.Context, uuid: str) -> None:
    ...
    if not result:
        embed = hikari.Embed(description='Card or pack was not found!', color=get_setting('general', 'embed_error_color'))
        await ctx.respond(embed, flags=hikari.MessageFlag.EPHEMERAL)
        return
    ...
    if name == 'Pack':
        ...
        embed = hikari.Embed(title=f'{pack_type} Pack', description='A standard pack that contains 7 Pokémon cards.' if pack_type == 'Standard' else 'A premium pack that contains 7 high quality Pokémon cards.', color=get_setting('general', 'embed_color'))
